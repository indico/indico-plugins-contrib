...
class RHEmailRepresentativesBase(RHAdminBase):
    ...
    def _process_args(self, affiliations):
        ...
        self.recipients = {
            ...
            for email in affiliation.contact_emails
...
class RHEmailRepresentativesMetadata(RHEmailRepresentativesBase):
    ...
    def _process(self):
        invalid_affiliations = [{
            'id': a.id,
            'invalid_emails': [
                email for email in a.contact_emails if email.strip().lower() not in self.recipients
            ],
...
class RHEmailRepresentativesSend(RHEmailRepresentativesBase):
    ...
    def _process(self, sender_address, body, subject, bcc_addresses, copy_for_sender):
        ...
        if not self.recipients:
            raise ExpectedError(_('There are no recipients with contact emails.'))
        ...
        for affiliation in self.affiliations:
            if not affiliation.contact_emails:
                continue
            ...
            tpl = get_plugin_template_module('emails/custom_email.html', subject=email_subject, body=email_body)
            email = make_email(to_list=affiliation.contact_emails, bcc_list=sorted(bcc), sender_address=sender_address,
